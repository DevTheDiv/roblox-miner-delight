...
class Mineable {
    ...
    constructor(position: Vector3 = new Vector3(0,0,0)){
        // @ts-ignore
        this.mesh = this.template.GetChildren()[0].Clone() as MeshPart;
        this.mesh.Parent = Workspace;
    ...
    private takeDamage(damage: number){
        ...
        if(this.health <= 66 && this.health > 33){
            console.log("Health at 66" );
            this.swapMesh(1);
        }
        if(this.health <= 33 && this.health > 0){
            console.log("Health at 33" );
            this.swapMesh(2);
        }
        if(this.health <= 0){
    ...
    private swapMesh(stage: number){
        ...
        this.stage = stage;
        // @ts-ignore
        let newMesh =  this.template.FindFirstChild(this.stages[stage]) as MeshPart;
